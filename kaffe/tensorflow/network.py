...
class Network(object):
        ...
        for fed_layer in args:
            if isinstance(fed_layer, str):
                try:
                    ...
                    self.multi_terminals.append(fed_layer)
                except KeyError:
                    raise KeyError('Unknown layer name fed: %s' % fed_layer)
            else:
                self.multi_terminals[-1] = fed_layer
    ...
    def deconv(self,
        ...
        assert c_o % group == 0
        # Convolution for a given input and kernel
        stride_shape = [s_h, s_w]
        kernel_shape = [k_h, k_w]
        input_shape = input.get_shape()[1:3]
        out_shape = [int(input.get_shape()[0])]
        for i in range(len(input_shape)):
            kernel_i = kernel_shape[i]
            stride_i = stride_shape[i]
            input_i = input_shape[i]
            if padding == 'SAME':
                out_shape.append(input_i*stride_i+kernel_i)
            else:
                out_shape.append(int((input_i-1)*stride_i+kernel_i))

        out_shape.append(int(c_o))

                # out_shape.append(math.floor((input_i + 2 * pad_i - kernel_i) / float(stride_i) + 1))
        deconv = lambda i, k: tf.nn.conv2d_transpose(i, k, output_shape=out_shape, strides=[1]+stride_shape+[1], padding=padding)

        # deconv = lambda i, k: tf.contrib.layers.conv2d_transpose(i,num_outputs=c_o,kernel_size=k,stride=stride_shape,padding=padding)
